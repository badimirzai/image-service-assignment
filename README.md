...
**Requirements:** Go version in go.mod (currently 1.25).
...
| [`internal/service/`](internal/service/) | Application logic: validation, metadata, crop, batch |
...
Architecture: thin handlers -> service -> store interface -> in-memory store (optional later: relational database management SQLite/PostgreSQL).
...
**Batch details:** `Content-Type: multipart/form-data` with one or more file parts named **`images`** (max **8**). Per-part and request size capped at ~10 MiB per image. Handler parses multipart; service uses a **4-worker** pool and the same validation as single create. Response is always **200** when the batch is accepted:

```json
{ "success": [ /* metadata... */ ], "errors": [ { "filename": "...", "error": "..." } ] }
```

Empty batch / too many parts -> **400**; whole request body too large -> **413**; a single oversized part is a per-item error in the **200** response (other images still processed). Not multipart -> **415**. On client disconnect, cancel flows through `r.Context()`: stop scheduling new jobs, skip further writes; cancel/validation failures appear in `errors`. Accepted batches still return **200** (including partial success / cancel); already-stored images may remain (no compensating deletes).
...
- **Thin handlers -> service -> store interface.** Enough seams for tests and a later SQLite swap; no DTO soup, middleware stack, or plugin system.
...
### Persistence (motivated tradeoff)

- **In-memory for this submission** so reviewers can clone/test/run with zero infra.
- **`store.Store` is the seam for durability:** handlers/service stay unchanged if you swap in SQLite (BLOB + columns) or object storage + a metadata DB.
- **Production scale path:** process-local memory will not horizontally scale or survive restarts -> blob/object store for bytes + metadata DB (+ CDN for reads). Batch already uses a fixed worker pool to bound CPU.
- **SQLite not shipped on purpose** - same interface, less eval friction and complexity. 
...
- **Limits (initial):** ~10 MiB per image; batch size capped at 8; 4 workers for batch.
...
| Code | When |
|------|------|
| `200` | List/get/PUT OK; batch accepted (full or partial success, including cancel-as-per-item errors) |
| `201` | Single create |
...
| Map safety | `sync.RWMutex` from day one | Correct concurrent practice; SQLite/PostgreSQL would use transactions later |
...
| Batch HTTP status | Always 200 when accepted | Per-item outcomes in body; avoid request-level 4xx that hides successful stores |
| Persistence | Memory now; Store interface ready | SQLite/object store later without rewriting handlers/service |

---

## Current status

**Done (in-memory):** all assignment endpoints, bbox cutout, concurrent batch with cancel via request context, tests.

**Explicitly deferred:** durable persistence (SQLite / object store behind `store.Store` / PostgreSQL), auth, delete, pagination, caching/ETags, deployment.